...
if __name__=='__main__':
    ...
    for j in tqdm(range(height)):
        for i in range(width):
            ...
            if ref_img[j,i]!=0:
                if pred_img[j,i]==0:
                    match_img[j,i]=4  # 漏检的目标
                if ref_img[j,i]==pred_img[j,i]:
                    match_img[j,i]=3 # true positive 检测正确的目标
                elif ref_img[j,i]==1 and pred_img[j,i]==2:
                    match_img[j,i]=5 # 道路被错分为房屋建筑
                elif ref_img[j,i]==2 and pred_img[j,i]==1:
                    match_img[j,i]=6 # 房屋建筑被错分为道路
            else:
                if pred_img[j, i] == 1:
                    match_img[j, i] = 1  # false negative 背景被错分为road
                if pred_img[j, i] == 2:
                    match_img[j, i] = 2
