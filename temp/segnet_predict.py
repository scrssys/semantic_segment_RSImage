...
def get_predicted_pathces_from_image(input_img, model, window_size, step, pre_func, labelencoder):
    ...
    assert channel < row
    subdivs = []
    for i in list(range(0,row-window_size+1, step)):
        subdivs.append([])
        for j in list(range (0, column-window_size+1, step)):
...
def mosaic_resut(predicted_patches):
    ...
    for i in list(range(0, patch_row*x, x)):
        b=0
        for j in list(range(0, patch_column*y, y)):
